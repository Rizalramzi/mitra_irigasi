  ...
  <body class="h-full flex overflow-hidden">
    ...
    <div class="flex flex-1 overflow-hidden h-full">
      ...
      <aside class="hidden md:flex md:w-80 flex-col bg-zinc-900 border-r border-zinc-800 shrink-0">
        ...
        <div class="p-6 border-b border-zinc-800">
          <div class="flex items-center gap-3">
            <div class="h-10 w-10 rounded-xl bg-emerald-600 flex items-center justify-center shadow-lg shadow-emerald-900/30">
        ...
        <div class="flex-1 p-6 space-y-6 overflow-y-auto custom-scrollbar">
          ...
          <div class="pt-4 border-t border-zinc-800">
            ...
            <div class="mt-2 space-y-2">
              <button onclick="sendQuickMessage('Apa saja layanan utama dari Mitra Irigasi?')" class="w-full text-left text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 p-2.5 rounded-lg border border-zinc-750 transition duration-150">
                "Apa saja layanan utama dari Mitra Irigasi?"
              </button>
              <button onclick="sendQuickMessage('Berapa lama garansi alat sensor Mitra-Moist?')" class="w-full text-left text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 p-2.5 rounded-lg border border-zinc-750 transition duration-150">
                "Berapa lama garansi alat sensor Mitra-Moist?"
              </button>
              <button onclick="sendQuickMessage('Bagaimana cara menghubungi tim support?')" class="w-full text-left text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 p-2.5 rounded-lg border border-zinc-750 transition duration-150">
      ...
      <main class="flex-1 flex flex-col bg-zinc-950 overflow-hidden relative">
        ...
        <!-- Header -->
        <header class="flex items-center justify-between px-6 py-4 border-b border-zinc-900 bg-zinc-900/40 backdrop-blur-md">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="font-semibold text-white leading-normal text-sm md:text-base">CS Assistant Mitra Irigasi</h1>
              <p class="text-xs text-zinc-400">Menyediakan info valid & solusi teknologi pengairan cerdas</p>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <span class="hidden md:inline-flex items-center px-2 py-1 rounded bg-zinc-800 text-zinc-400 text-xs font-mono font-medium">Model: GEMINI-2.5</span>
            <button onclick="clearChat()" class="text-zinc-400 hover:text-white p-2 rounded-lg hover:bg-zinc-800 transition duration-150" title="Bersihkan Chat">
        ...
        <div id="messagesContainer" class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          ...
          <div class="flex gap-4 max-w-3xl">
            <div class="h-8 w-8 rounded bg-emerald-600 flex items-center justify-center shrink-0 shadow-lg text-white">
          ...
          <div id="suggestionChips" class="flex flex-wrap gap-2 pt-2 max-w-3xl pl-12">
            <button onclick="sendQuickMessage('Jelaskan detail mengenai produk sensor Mitra-Moist!')" class="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 text-xs px-3.5 py-2 rounded-full border border-zinc-850 hover:border-zinc-700 transition duration-150">
              💡 Detail Mitra-Moist
            </button>
            <button onclick="sendQuickMessage('Apakah ada garansi sistem otomatisasi alat?')" class="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 text-xs px-3.5 py-2 rounded-full border border-zinc-850 hover:border-zinc-700 transition duration-150">
              🛡️ Informasi Garansi
            </button>
            <button onclick="sendQuickMessage('Di mana alamat kantor Mitra Irigasi?')" class="bg-zinc-900 hover:bg-zinc-800 text-zinc-300 text-xs px-3.5 py-2 rounded-full border border-zinc-850 hover:border-zinc-700 transition duration-150">
          ...
          <div id="typingIndicator" class="hidden gap-4 max-w-3xl">
            ...
            <div class="bg-zinc-900 border border-zinc-800/80 rounded-2xl rounded-tl-sm px-5 py-4 shadow-md flex items-center gap-1.5 min-w-17.5 justify-center">
        ...
        <div class="p-6 border-t border-zinc-900 bg-zinc-905">
          <form id="chatForm" onsubmit="handleChatSubmit(event)" class="max-w-3xl mx-auto relative flex items-center">
            <input 
              type="text" 
              id="messageInput"
              placeholder="Ketik pertanyaan tentang Mitra Irigasi..."
    ...
    <script>
      ...
      async function handleChatSubmit(event) {
        ...
        messageHistory.push({ sender: 'user', text: query });
        ...
        try {
          ...
          if (response.ok && data.reply) {
            ...
            messageHistory.push({ sender: 'bot', text: data.reply });
          } else {
            const errorText = data.reply || "Maaf, terjadi ketidakcocokan data di server. Coba kirim ulang pesan Anda.";
            appendMessage('bot', `⚠️ ${errorText}`);
          }
        } catch (error) {
          typingBox.classList.add('hidden');
          typingBox.classList.remove('flex');
          appendMessage('bot', "⚠️ Gagal mengirim pesan. Pastikan sambungan jaringan Anda aktif dan server web berjalan.");
